...
export default function ArgentinaMap({ setRegion, setLengthOfCountriesToFinishGaming }){
    ...
    useEffect(()=>{
        if(Object.keys(arMill.content.paths).length === 0) return;        
        setLengthOfCountriesToFinishGaming(Object.keys(arMill.content.paths).length)
    },[]);

    return(
        <VectorMap map={arMill} series={{
            regions: [{
                scale: ["#E2AEFF", "#5E32CA"],
