...
export default function AustriaMap({ setRegion, setLengthOfCountriesToFinishGaming }){
    ...
    useEffect(()=>{
        if(Object.keys(atMill.content.paths).length === 0) return;        
        setLengthOfCountriesToFinishGaming(Object.keys(atMill.content.paths).length)
    },[]);

    return(
        <VectorMap map={atMill} series={{
            regions: [{
                scale: ["#E2AEFF", "#5E32CA"],
