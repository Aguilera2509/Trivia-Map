...
export default function ItalyMap({ setRegion, setLengthOfCountriesToFinishGaming }){
    ...
    useEffect(()=>{
        if(Object.keys(itMill.content.paths).length === 0) return;        
        setLengthOfCountriesToFinishGaming(Object.keys(itMill.content.paths).length)
    },[]);

    return(
        <VectorMap map={itMill} series={{
            regions: [{
                scale: ["#E2AEFF", "#5E32CA"],
