...
export default function AfricaMap({ setRegion, setLengthOfCountriesToFinishGaming }){
    ...
    useEffect(()=>{
        if(Object.keys(africaMill.content.paths).length === 0) return;        
        setLengthOfCountriesToFinishGaming(Object.keys(africaMill.content.paths).length)
    },[]);

    return(
        <VectorMap map={africaMill} series={{
            regions: [{
                scale: ["#E2AEFF", "#5E32CA"],
