...
export default function ChinaMap({ setRegion, setLengthOfCountriesToFinishGaming }){
    ...
    useEffect(()=>{
        if(Object.keys(cnMill.content.paths).length === 0) return;        
        setLengthOfCountriesToFinishGaming(Object.keys(cnMill.content.paths).length)
    },[]);

    return(
        <VectorMap map={cnMill} series={{
            regions: [{
                scale: ["#E2AEFF", "#5E32CA"],
