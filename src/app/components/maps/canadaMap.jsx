...
export default function CanadaMap({ setRegion, setLengthOfCountriesToFinishGaming }){
    ...
    useEffect(()=>{
        if(Object.keys(caMill.content.paths).length === 0) return;        
        setLengthOfCountriesToFinishGaming(Object.keys(caMill.content.paths).length)
    },[]);

    return(
        <VectorMap map={caMill} series={{
            regions: [{
                scale: ["#E2AEFF", "#5E32CA"],
