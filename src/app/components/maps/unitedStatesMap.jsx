...
export default function UnitedStatesMap({ setRegion, setLengthOfCountriesToFinishGaming }){
    ...
    useEffect(()=>{
        if(Object.keys(usMill.content.paths).length === 0) return;        
        setLengthOfCountriesToFinishGaming(Object.keys(usMill.content.paths).length)
    },[]);

    return(
        <VectorMap map={usMill} series={{
            regions: [{
                scale: ["#E2AEFF", "#5E32CA"],
