...
export default function SwedenMap({ setRegion, setLengthOfCountriesToFinishGaming  }){
    ...
    useEffect(()=>{
        if(Object.keys(seMill.content.paths).length === 0) return;        
        setLengthOfCountriesToFinishGaming(Object.keys(seMill.content.paths).length)
    },[]);

    return(
        <VectorMap map={seMill} series={{
            regions: [{
                scale: ["#E2AEFF", "#5E32CA"],
