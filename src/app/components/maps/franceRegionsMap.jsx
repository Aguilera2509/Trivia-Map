...
export default function FranceRegionsMap({ setRegion, setLengthOfCountriesToFinishGaming }){
    ...
    useEffect(()=>{
        if(Object.keys(frRegionsMill.content.paths).length === 0) return;        
        setLengthOfCountriesToFinishGaming(Object.keys(frRegionsMill.content.paths).length)
    },[]);

    return(
        <VectorMap map={frRegionsMill} series={{
            regions: [{
                scale: ["#E2AEFF", "#5E32CA"],
