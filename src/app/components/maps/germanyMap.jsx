...
export default function GermanyMap({ setRegion, setLengthOfCountriesToFinishGaming  }){
    ...
    useEffect(()=>{
        if(Object.keys(deMill.content.paths).length === 0) return;        
        setLengthOfCountriesToFinishGaming(Object.keys(deMill.content.paths).length)
    },[]);

    return(
        <VectorMap map={deMill} series={{
            regions: [{
                scale: ["#E2AEFF", "#5E32CA"],
