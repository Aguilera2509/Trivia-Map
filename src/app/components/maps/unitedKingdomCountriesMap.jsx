...
export default function UnitedKingdomCountriesMap({ setRegion, setLengthOfCountriesToFinishGaming }){
    ...
    useEffect(()=>{
        if(Object.keys(ukCountriesMill.content.paths).length === 0) return;        
        setLengthOfCountriesToFinishGaming(Object.keys(ukCountriesMill.content.paths).length)
    },[]);

    return(
        <VectorMap map={ukCountriesMill} series={{
            regions: [{
                scale: ["#E2AEFF", "#5E32CA"],
