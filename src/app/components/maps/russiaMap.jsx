...
export default function RussiaMap({ setRegion, setLengthOfCountriesToFinishGaming  }){
    ...
    useEffect(()=>{
        if(Object.keys(ruMill.content.paths).length === 0) return;        
        setLengthOfCountriesToFinishGaming(Object.keys(ruMill.content.paths).length)
    },[]);

    return(
        <VectorMap map={ruMill} series={{
            regions: [{
                scale: ["#E2AEFF", "#5E32CA"],
