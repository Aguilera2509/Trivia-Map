...
export default function TurkeyMap({ setRegion, setLengthOfCountriesToFinishGaming  }){
    ...
    useEffect(()=>{
        if(Object.keys(trMill.content.paths).length === 0) return;        
        setLengthOfCountriesToFinishGaming(Object.keys(trMill.content.paths).length)
    },[]);

    return(
        <VectorMap map={trMill} series={{
            regions: [{
                scale: ["#E2AEFF", "#5E32CA"],
