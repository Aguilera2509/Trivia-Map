...
export default function EuropeMap({ setRegion, setLengthOfCountriesToFinishGaming }){
    ...
    useEffect(()=>{
        if(Object.keys(europeMill.content.paths).length === 0) return;        
        setLengthOfCountriesToFinishGaming(Object.keys(europeMill.content.paths).length)
    },[]);

    return(
        <VectorMap map={europeMill} series={{
            regions: [{
                scale: ["#E2AEFF", "#5E32CA"],
