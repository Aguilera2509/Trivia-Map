...
export default function UkRegionsMap({ setRegion, setLengthOfCountriesToFinishGaming }){
    ...
    useEffect(()=>{
        if(Object.keys(ukRegionsMill.content.paths).length === 0) return;        
        setLengthOfCountriesToFinishGaming(Object.keys(ukRegionsMill.content.paths).length)
    },[]);

    return(
        <VectorMap map={ukRegionsMill} series={{
            regions: [{
                scale: ["#E2AEFF", "#5E32CA"],
