...
export default function AlgeriaMap({ setRegion, setLengthOfCountriesToFinishGaming }){
    ...
    useEffect(()=>{
        if(Object.keys(dzMill.content.paths).length === 0) return;        
        setLengthOfCountriesToFinishGaming(Object.keys(dzMill.content.paths).length)
    },[]);

    return(
        <VectorMap map={dzMill} series={{
            regions: [{
                scale: ["#E2AEFF", "#5E32CA"],
